...
#define STOP_TILER(suffix, TileIndex, rot) ( \
							(m_TileIndex ## suffix == TileIndex && m_TileFlags ## suffix == ROTATION_ ## rot) \
							|| (m_TileFIndex ## suffix == TileIndex && m_TileFFlags ## suffix == ROTATION_ ## rot) \
							|| (m_TileSIndex ## suffix == TileIndex && m_TileSFlags ## suffix == ROTATION_ ## rot) \
							)
#define STOP_TILE(suffix, TileIndex) (m_TileIndex ## suffix == TileIndex || m_TileFIndex ## suffix == TileIndex || m_TileSIndex ## suffix == TileIndex)

					if(Temp.x > 0 &&
					   (STOP_TILER(, TILE_STOP, 270) || STOP_TILER(L, TILE_STOP, 270)
						|| STOP_TILER(L, TILE_STOPS, 270) || STOP_TILER(L, TILE_STOPS, 90)
						|| STOP_TILE(L, TILE_STOPA)
					   ))
						Temp.x = 0;
					if(Temp.x < 0 &&
					...
					if(Temp.y < 0 &&
					   (STOP_TILER(, TILE_STOP, 180) || STOP_TILER(B, TILE_STOP, 180)
						|| STOP_TILER(B, TILE_STOPS, 0) || STOP_TILER(B, TILE_STOPS, 180)
						|| STOP_TILE(B, TILE_STOPA)
					   ))
						Temp.y = 0;
					if(Temp.y > 0 &&
					   (STOP_TILER(, TILE_STOP, 0) || STOP_TILER(T, TILE_STOP, 0)
						|| STOP_TILER(T, TILE_STOPS, 0) || STOP_TILER(T, TILE_STOPS, 180)
						|| STOP_TILE(T, TILE_STOPA)
					   ))
						Temp.y = 0;
