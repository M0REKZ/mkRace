...
void CGameContext::SendChat(int ChatterClientID, int Mode, int To, const char *pText)
{
	...
	if(Mode == CHAT_ALL && !(Config()->m_SvNoChat & 1))
		Server()->SendPackMsg(&Msg, MSGFLAG_VITAL, To);
	else if(Mode == CHAT_TEAM && !(Config()->m_SvNoChat & 2))
	{
	...
	else if(Mode == CHAT_WHISPER && !(Config()->m_SvNoChat & 4))
	{
		// send to the clients
		Msg.m_TargetID = To;
		if(ChatterClientID >= 0 && ChatterClientID < MAX_CLIENTS)
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	NewClientInfoMsg.m_pName = Config()->m_SvNoNames ? "" : Server()->ClientName(ClientID);
	NewClientInfoMsg.m_pClan = Server()->ClientClan(ClientID);
	NewClientInfoMsg.m_Country = Server()->ClientCountry(ClientID);
	NewClientInfoMsg.m_Silent = Config()->m_SvSilentLogin;
	...
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		ClientInfoMsg.m_pName = Config()->m_SvNoNames ? "" : Server()->ClientName(i);
...
void CGameContext::OnClientDrop(int ClientID, const char *pReason)
{
	...
	if(Server()->ClientIngame(ClientID) || (m_apPlayers[ClientID] && m_apPlayers[ClientID]->IsDummy()) || IsClientBot(ClientID))
	{
		if(Server()->DemoRecorder_IsRecording())
		{
		...
		Msg.m_ClientID = ClientID;
		Msg.m_pReason = pReason;
		Msg.m_Silent = Config()->m_SvSilentLogin;
		if((Config()->m_SvSilentSpectatorMode && m_apPlayers[ClientID]->GetTeam() == TEAM_SPECTATORS) || m_apPlayers[ClientID]->IsDummy())
			Msg.m_Silent = true;
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
				...
				if(pChar)
				{
					...
					if(DummyId)
					{
						...
						if(pChar)
						{
							...
							NewClientInfoMsg.m_pName = Config()->m_SvNoNames ? "" : Server()->ClientName(DummyId);
		...
		else if (MsgID == NETMSGTYPE_CL_SKINCHANGE)
		{
			...
			for(int p = 0; p < NUM_SKINPARTS; p++)
			{
				str_copy(pPlayer->m_TeeInfos.m_aaSkinPartNames[p], pMsg->m_apSkinPartNames[p], MAX_SKIN_LENGTH);
...
void CGameContext::OnInit()
{
	...
	Kernel()->RequestInterface<IConfigManager>()->Reset(CFGFLAG_MAPSETTINGS);
	LoadMapSettings();
